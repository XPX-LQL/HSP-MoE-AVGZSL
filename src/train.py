...
def add_logs_tensorboard(batch_loss_details, writer, batch_idx, step, which_stage):
    loss_keys = [
        ...
        'Loss/loss_cmd_rec',
        'Loss/cross_entropy',
        'Loss/semantic_distill',
        'Loss/semantic_hup',
    ]
    for key in loss_keys:
        if key in batch_loss_details:
            writer.add_scalar(f"{key}_{which_stage}", batch_loss_details[key] / batch_idx, step)


def _semantic_distill_component_weight(args, component):
    weight = getattr(args, f"semantic_{component}_weight", None)
    if weight is None:
        weight = getattr(args, "semantic_distill_weight", 0.0)
    return weight or 0.0


def _semantic_distill_enabled(args):
    if not getattr(args, "semantic_distill_loss", False):
        return False

    mode = getattr(args, "semantic_distill_mode", "distribution")
    distribution_weight = _semantic_distill_component_weight(args, "distribution")
    relation_weight = _semantic_distill_component_weight(args, "relation")

    if mode == "distribution":
        return distribution_weight > 0
    if mode == "relation":
        return relation_weight > 0
    return (
...
def _semantic_distill_needs_class_embeddings(args):
    return (
            _semantic_distill_enabled(args)
            and getattr(args, "semantic_distill_mode", "distribution") in ("distribution", "both")
            and _semantic_distill_component_weight(args, "distribution") > 0
    )


def _needs_class_embeddings(args):
    return (
            args.cross_entropy_loss
            or _semantic_distill_needs_class_embeddings(args)
    )


def _needs_class_index(args):
    return args.cross_entropy_loss
...
def train_step(data_loader, model, criterion, optimizer, epoch, epochs, writer, device, metrics, stats,  args):
    ...
    class_embeddings = embeddings if _needs_class_embeddings(args) else None
